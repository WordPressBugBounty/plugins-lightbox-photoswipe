...
let hideScrollbar=function(){const scrollbarWidth=window.innerWidth-document.body.offsetWidth;document.body.style.paddingRight=scrollbarWidth+'px';document.body.style.overflow='hidden'};let showScrollbar=function(){document.body.style.paddingRight=originalBodyPaddingRight;document.body.style.overflow=originalBodyOverflow};let sanitizeCaption=function(caption){const regexp=/(?:(?!(<span class="([^"]+)">)|(<\/span>)|(<br>)|(<br \/>)|(<b>)|(<\/b>)|(<i>)|(<\/i>)|(<em>)|(<\/em>)|(<strong>)|(<\/strong>)|(<div>)|(<div style="([^"]+)">)|(<div class="([^"]+)">)|(<\/div>))(<[^>]+>))*/gi;if(typeof caption==='string'||caption instanceof String){return caption.replaceAll(regexp,'')}
return caption}
let parseThumbnailElements=function(link,id){let elements,galleryItems=[],index;if(id==null||id==1){elements=document.querySelectorAll('a[data-lbwps-width]:not([data-lbwps-gid])')}else{elements=document.querySelectorAll('a[data-lbwps-width][data-lbwps-gid="'+id+'"]')}
...
title='';if(element.getAttribute('data-lbwps-title')!=null&&element.getAttribute('data-lbwps-title')!=''){title=title+'<div class="pswp__caption__title">'+sanitizeCaption(element.getAttribute('data-lbwps-title'))+'</div>'}
if(lbwpsOptions.usecaption==='1'&&caption!=null&&caption!=''){title=title+'<div class="pswp__caption__text">'+sanitizeCaption(caption)+'</div>'}
let alt='';if(element.firstElementChild){alt=element.firstElementChild.getAttribute('alt');if(!alt){alt=''}}
if(lbwpsOptions.use_alt==='1'&&alt!==''){title=title+'<div class="pswp__caption__alt">'+sanitizeCaption(element.firstElementChild.getAttribute('alt'))+'</div>'}
if(element.getAttribute('data-lbwps-description')!=null){title=title+'<div class="pswp__caption__desc">'+sanitizeCaption(element.getAttribute('data-lbwps-description'))+'</div>'}
if(element.getAttribute('data-lbwps-filename')!=null){title=title+'<div class="pswp__caption__file">'+sanitizeCaption(element.getAttribute('data-lbwps-filename'))+'</div>'}
...
exif=sanitizeCaption(exif);if(alt===''){alt=caption}
alt=sanitizeCaption(alt);galleryItems.push({src:element.getAttribute('href'),msrc:msrc,width:element.getAttribute('data-lbwps-width'),height:element.getAttribute('data-lbwps-height'),title:title,exif:exif,el:element,tabindex:tabindex,alt:alt})}}
